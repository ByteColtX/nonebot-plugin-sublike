...
| 配置项 | 必填 | 默认值 | 说明 |
| :---: | :---: | :---: | :--- |
| `sublike_cmd_me` | 否 | `["赞我", "草我", "超我"]` | 即时点赞自己的触发词 |
| `sublike_cmd_other` | 否 | `["赞ta", "赞TA", "赞他", "赞她", "草ta", "草TA", "草他", "草她"]` | 即时点赞他人的触发词 |
| `sublike_cmd_sub` | 否 | `["每日赞", "天天赞我"]` | 每日赞触发词 |
| `sublike_cmd_unsub` | 否 | `["取消每日赞", "每日赞取消"]` | 关闭每日赞触发词 |
| `sublike_cmd_status` | 否 | `["每日赞查看", "查看每日赞", "每日赞状态", "每日赞查询", "查询每日赞"]` | 每日赞状态查询触发词 |
| `sublike_need_friend_me` | 否 | `false` | 即时点赞是否要求好友关系 |
| `sublike_need_friend_sub` | 否 | `true` | 每日赞是否要求好友关系 |
| `sublike_allow_other` | 否 | `true` | 是否允许即时点赞他人 |
| `sublike_sub_expire_days` | 否 | `7` | 每日赞有效期天数，需在过期前再次触发命令续期 |
| `sublike_sched_interval` | 否 | `60` | 定时扫描间隔，单位分钟 |
| `sublike_sched_start` | 否 | `8` | 定时任务开始小时 |
| `sublike_sched_end` | 否 | `0` | 定时任务结束小时，`0` 表示次日 `00:00` |
| `sublike_delay_max` | 否 | `2` | 单个每日赞用户执行前的最大随机延迟，单位分钟 |
...
| `每日赞` / `天天赞我` | 群员 | 否 | 群聊 / 私聊 | 开启或续期每日赞 |
| `取消每日赞` / `每日赞取消` | 群员 | 否 | 群聊 / 私聊 | 关闭自己的每日赞 |
| `每日赞查看` / `查看每日赞` / `每日赞状态` / `每日赞查询` / `查询每日赞` | 群员 | 否 | 群聊 / 私聊 | 普通用户查看自己的每日赞状态，`SUPERUSERS` 可查看全部有效每日赞 |
